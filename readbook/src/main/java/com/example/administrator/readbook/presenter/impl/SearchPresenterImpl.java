...
import com.example.administrator.readbook.dao.SearchHistoryBeanDao;
import com.example.administrator.readbook.listener.OnGetChapterListListener;
import com.example.administrator.readbook.model.impl.GxwztvBookModelImpl;
import com.example.administrator.readbook.model.impl.LingdiankanshuStationBookModelImpl;
import com.example.administrator.readbook.model.impl.WebBookModelImpl;
import com.example.administrator.readbook.presenter.ISearchPresenter;
import com.example.administrator.readbook.utils.NetworkUtil;
import com.example.administrator.readbook.view.ISearchView;
    ...
    public SearchPresenterImpl() {
        Observable.create(new ObservableOnSubscribe<List<BookShelfBean>>() {
            ...
            public void subscribe(ObservableEmitter<List<BookShelfBean>> e) throws Exception {
                List<BookShelfBean> temp = DbHelper.getInstance(mView.getContext()).getmDaoSession().getBookShelfBeanDao().queryBuilder().list();
                if (temp == null)
    ...
    public void insertSearchHistory() {
        ...
        Observable.create(new ObservableOnSubscribe<SearchHistoryBean>() {
            ...
            public void subscribe(ObservableEmitter<SearchHistoryBean> e) throws Exception {
                List<SearchHistoryBean> datas = DbHelper.getInstance(mView.getContext()).getmDaoSession().getSearchHistoryBeanDao()
                        .queryBuilder()
                ...
                if (null != datas && datas.size() > 0) {
                    ...
                    DbHelper.getInstance(mView.getContext()).getmDaoSession().getSearchHistoryBeanDao().update(searchHistoryBean);
                } else {
                    searchHistoryBean = new SearchHistoryBean(type, content, System.currentTimeMillis());
                    DbHelper.getInstance(mView.getContext()).getmDaoSession().getSearchHistoryBeanDao().insert(searchHistoryBean);
                }
    ...
    public void cleanSearchHistory() {
        ...
        Observable.create(new ObservableOnSubscribe<Integer>() {
            ...
            public void subscribe(ObservableEmitter<Integer> e) throws Exception {
                int a = DbHelper.getInstance(mView.getContext()).getDb().delete(SearchHistoryBeanDao.TABLENAME, SearchHistoryBeanDao.Properties.Type.columnName + "=? and " + SearchHistoryBeanDao.Properties.Content.columnName + " like ?", new String[]{String.valueOf(type), "%" + content + "%"});
                e.onNext(a);
            }
        }).subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
    ...
    private void saveBookToShelf(final BookShelfBean bookShelfBean) {
        Observable.create(new ObservableOnSubscribe<BookShelfBean>() {
            ...
            public void subscribe(ObservableEmitter<BookShelfBean> e) throws Exception {
                DbHelper.getInstance(mView.getContext()).getmDaoSession().getChapterListBeanDao().insertOrReplaceInTx(bookShelfBean.getBookInfoBean().getChapterlist());
                DbHelper.getInstance(mView.getContext()).getmDaoSession().getBookInfoBeanDao().insertOrReplace(bookShelfBean.getBookInfoBean());
                //网络数据获取成功  存入BookShelf表数据库
                DbHelper.getInstance(mView.getContext()).getmDaoSession().getBookShelfBeanDao().insertOrReplace(bookShelfBean);
