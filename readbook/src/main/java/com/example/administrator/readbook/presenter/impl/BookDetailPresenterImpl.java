...
import com.example.administrator.readbook.presenter.IBookDetailPresenter;
import com.example.administrator.readbook.view.IBookDetailView;
                    ...
                    .subscribe(new SimpleObserver<Boolean>() {
                        ...
                        public void onNext(Boolean value) {
                            if (value) {
                                ...
                                Toast.makeText(mView.getContext(), "放入书架失败!", Toast.LENGTH_SHORT).show();
                            }
                        }
    ...
    public void removeFromBookShelf() {
        if (bookShelf != null) {
            Observable.create(new ObservableOnSubscribe<Boolean>() {
                @Override
                public void subscribe(ObservableEmitter<Boolean> e) throws Exception {
                    DbHelper.getInstance(mView.getContext()).getmDaoSession().getBookShelfBeanDao().deleteByKey(bookShelf.getNoteUrl());
                    DbHelper.getInstance(mView.getContext()).getmDaoSession().getBookInfoBeanDao().deleteByKey(bookShelf.getBookInfoBean().getNoteUrl());
                    ...
                    DbHelper.getInstance(mView.getContext()).getmDaoSession().getBookContentBeanDao().deleteByKeyInTx(keys);
                    DbHelper.getInstance(mView.getContext()).getmDaoSession().getChapterListBeanDao().deleteInTx(bookShelf.getBookInfoBean().getChapterlist());
                    e.onNext(true);
                    e.onComplete();
                }
            }).subscribeOn(Schedulers.newThread())
