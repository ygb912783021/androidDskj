...
package com.example.administrator.readbook.service;
...
import com.example.administrator.readbook.bean.DownloadChapterBean;
import com.example.administrator.readbook.bean.DownloadChapterListBean;
import com.example.administrator.readbook.common.RxBusTag;
import com.example.administrator.readbook.dao.BookContentBeanDao;
import com.example.administrator.readbook.dao.BookShelfBeanDao;
import com.example.administrator.readbook.dao.DbHelper;
import com.example.administrator.readbook.dao.DownloadChapterBeanDao;
import com.example.administrator.readbook.model.impl.WebBookModelImpl;
    ...
    private void addNewTask(final List<DownloadChapterBean> newData) {
        ...
        Observable.create(new ObservableOnSubscribe<Boolean>() {
            ...
            public void subscribe(ObservableEmitter<Boolean> e) throws Exception {
                DbHelper.getInstance(getApplicationContext()).getmDaoSession().getDownloadChapterBeanDao().insertOrReplaceInTx(newData);
                e.onNext(true);
                e.onComplete();
            }
        })
        ...
        if (isStartDownload) {
            Observable.create(new ObservableOnSubscribe<DownloadChapterBean>() {
                ...
                public void subscribe(ObservableEmitter<DownloadChapterBean> e) throws Exception {
                    List<BookShelfBean> bookShelfBeanList = DbHelper.getInstance(getApplicationContext()).getmDaoSession().getBookShelfBeanDao().queryBuilder().orderDesc(BookShelfBeanDao.Properties.FinalDate).list();
                    if (bookShelfBeanList != null && bookShelfBeanList.size() > 0) {
                        for (BookShelfBean bookItem : bookShelfBeanList) {
                            if (!bookItem.getTag().equals(BookShelfBean.LOCAL_TAG)) {
                                List<DownloadChapterBean> downloadChapterList = DbHelper.getInstance(getApplicationContext()).getmDaoSession().getDownloadChapterBeanDao().queryBuilder().where(DownloadChapterBeanDao.Properties.NoteUrl.eq(bookItem.getNoteUrl())).orderAsc(DownloadChapterBeanDao.Properties.DurChapterIndex).limit(1).list();
                        ...
                        DbHelper.getInstance(getApplicationContext()).getmDaoSession().getDownloadChapterBeanDao().deleteAll();
        ...
        if (durTime < reTryTimes && isStartDownload) {
            ...
            Observable.create(new ObservableOnSubscribe<BookContentBean>() {
                ...
                public void subscribe(ObservableEmitter<BookContentBean> e) throws Exception {
                    List<BookContentBean> result = DbHelper.getInstance(getApplicationContext()).getmDaoSession().getBookContentBeanDao().queryBuilder().where(BookContentBeanDao.Properties.DurChapterUrl.eq(data.getDurChapterUrl())).list();
                    ...
                    if (bookContentBean.getDurChapterUrl() == null || bookContentBean.getDurChapterUrl().length() <= 0) {
                        return WebBookModelImpl.getInstance().getBookContent(data.getDurChapterUrl(), data.getDurChapterIndex(), data.getTag()).map(new Function<BookContentBean, BookContentBean>() {
                            ...
                            public BookContentBean apply(BookContentBean bookContentBean) throws Exception {
                                DbHelper.getInstance(getApplicationContext()).getmDaoSession().getDownloadChapterBeanDao().delete(data);
                                if (bookContentBean.getRight()) {
                                    DbHelper.getInstance(getApplicationContext()).getmDaoSession().getBookContentBeanDao().insertOrReplace(bookContentBean);
                                    DbHelper.getInstance(getApplicationContext()).getmDaoSession().getChapterListBeanDao().update(new ChapterListBean(data.getNoteUrl(), data.getDurChapterIndex(), data.getDurChapterUrl(), data.getDurChapterName(), data.getTag(), true));
                                }
